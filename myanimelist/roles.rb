module Roles
  ...
  self.anime_roles = anime_string.split(',').map(&:to_i) unless anime_string.nil? # else update_roles = []

  manga_string = db.execute(
    'SELECT roles FROM perms WHERE command = ?', 'manga'
  )[0][0]
  self.manga_roles = manga_string.split(',').map(&:to_i) unless manga_string.nil? # else update_roles = []
