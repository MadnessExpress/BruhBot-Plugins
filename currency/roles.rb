module Roles
  ...
  self.money_roles = money_string.split(',').map(&:to_i) unless money_string.nil? # else update_roles = []

  money_give_string = db.execute(
    'SELECT roles FROM perms WHERE command = ?', 'money.give'
  )[0][0]
  self.money_give_roles = money_give_string.split(',').map(&:to_i) unless money_give_string.nil? # else update_roles = []

  money_add_string = db.execute(
    'SELECT roles FROM perms WHERE command = ?', 'money.add'
  )[0][0]
  self.money_add_roles = money_add_string.split(',').map(&:to_i) unless money_add_string.nil? # else update_roles = []
