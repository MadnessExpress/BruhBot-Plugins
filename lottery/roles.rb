module Roles
  ...
  self.lotto_start_roles = lotto_start_string.split(',').map(&:to_i) unless lotto_start_string.nil? # else update_roles = []

  lotto_enter_string = db.execute(
    'SELECT roles FROM perms WHERE command = ?', 'lotto.enter'
  )[0][0]
  self.lotto_enter_roles = lotto_enter_string.split(',').map(&:to_i) unless lotto_enter_string.nil? # else update_roles = []

  lotto_end_string = db.execute(
    'SELECT roles FROM perms WHERE command = ?', 'lotto.end'
  )[0][0]
  self.lotto_end_roles = lotto_end_string.split(',').map(&:to_i) unless lotto_end_string.nil? # else update_roles = []

  lotto_kill_string = db.execute(
    'SELECT roles FROM perms WHERE command = ?', 'lotto.kill'
  )[0][0]
  self.lotto_kill_roles = lotto_kill_string.split(',').map(&:to_i) unless lotto_kill_string.nil? # else update_roles = []
