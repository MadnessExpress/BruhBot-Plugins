...
say_roles = say_string.split(',').map(&:to_i) unless say_string.nil? # else update_roles = []

say_channel_string = db.execute(
  'SELECT roles FROM perms WHERE command = ?', 'say.channel'
)[0][0]
say_channel_roles = say_channel_string.split(',').map(&:to_i) unless say_channel_string.nil? # else update_roles = []
