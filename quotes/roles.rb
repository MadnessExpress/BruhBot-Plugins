module Roles
  ...
  self.quote_roles = quote_string.split(',').map(&:to_i) unless quote_string.nil? # else update_roles = []

  quote_add_string = db.execute(
    'SELECT roles FROM perms WHERE command = ?', 'quote.add'
  )[0][0]
  self.quote_add_roles = quote_add_string.split(',').map(&:to_i) unless quote_add_string.nil? # else update_roles = []

  quote_remove_string = db.execute(
    'SELECT roles FROM perms WHERE command = ?', 'quote.remove'
  )[0][0]
  self.quote_remove_roles = quote_remove_string.split(',').map(&:to_i) unless quote_remove_string.nil? # else update_roles = []
