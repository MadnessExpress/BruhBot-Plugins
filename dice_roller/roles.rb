...
roll_roles = roll_string.split(',').map(&:to_i) unless roll_string.nil? # else update_roles = []

roll_mod_string = db.execute(
  'SELECT roles FROM perms WHERE command = ?', 'roll.mod'
)[0][0]
roll_mod_roles = roll_mod_string.split(',').map(&:to_i) unless roll_mod_string.nil? # else update_roles = []

roll_fudge_string = db.execute(
  'SELECT roles FROM perms WHERE command = ?', 'roll.fudge'
)[0][0]
roll_fudge_roles = roll_fudge_string.split(',').map(&:to_i) unless roll_fudge_string.nil? # else update_roles = []

coin_string = db.execute(
  'SELECT roles FROM perms WHERE command = ?', 'coin'
)[0][0]
coin_roles = coin_string.split(',').map(&:to_i) unless coin_string.nil? # else update_roles = []
